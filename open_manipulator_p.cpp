...
void OpenManipulator::initOpenManipulator(bool using_actual_robot_state, STRING usb_port, STRING baud_rate, float control_loop_time, bool with_gripper, int ikSolver)
{
  /*****************************************************************************
  ...
  *****************************************************************************/
  if (ikSolver == 0)
  {
	kinematics_ = new kinematics::SolverUsingCRAndGeometry(); //default
  }
  else
  {
	  kinematics_ = new kinematics::SolverUsingHyuns(); //hyun's ik
	  printf("Hyun's I.K.\n");
  }

  //kinematics_ = new kinematics::SolverUsingCRAndGeometry(); //default

  //kinematics_ = new kinematics::SolverUsingCRAndSRJacobian(); //failed
  //kinematics_ = new kinematics::SolverUsingCRAndJacobian(); //Position Ok Orientation Failed
  //kinematics_ = new kinematics::SolverCustomizedforOMChain(); //Position Ok Orientation Failed
