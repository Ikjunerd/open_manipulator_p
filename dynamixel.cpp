...
bool JointDynamixel::writeGoalPosition(std::vector<uint8_t> actuator_id, std::vector<double> radian_vector)
{
  ...
  std::vector<uint8_t> id_array(actuator_id.size());
  std::vector<int32_t> goal_position(actuator_id.size());
  ...
  result = dynamixel_workbench_->syncWrite(SYNC_WRITE_HANDLER, id_array.data(), actuator_id.size(), goal_position.data(), 1, &log);
  if (result == false)
  {
    log::error(log);
  }
...
std::vector<robotis_manipulator::ActuatorValue> JointDynamixel::receiveAllDynamixelValue(std::vector<uint8_t> actuator_id)
{
  ...
  std::vector<int32_t> get_current(actuator_id.size());
  std::vector<int32_t> get_velocity(actuator_id.size());
  std::vector<int32_t> get_position(actuator_id.size());
...
bool JointDynamixelProfileControl::writeGoalProfilingControlValue(std::vector<uint8_t> actuator_id, std::vector<robotis_manipulator::ActuatorValue> value_vector)
{
  ...
  std::vector<uint8_t> id_array(actuator_id.size());
  std::vector<int32_t> goal_value(actuator_id.size());
  ...
  result = dynamixel_workbench_->syncWrite(SYNC_WRITE_HANDLER, id_array.data(), actuator_id.size(), goal_value.data(), 1, &log);
  if (result == false)
  {
    log::error(log);
  }
