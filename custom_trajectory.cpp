...
TaskWaypoint Rhombus::drawRhombus(double tick)
{
  ...
  while(true)
  {
    if (get_time_var < M_PI*2) break;
    get_time_var = get_time_var - M_PI*2;
  }

  if (get_time_var >= 0 && get_time_var < M_PI_2){
    traj[0] = - get_time_var / M_PI_2;
    traj[1] = - get_time_var / M_PI_2;
  } else if (get_time_var >= M_PI_2 && get_time_var < M_PI){
    traj[0] = - get_time_var / M_PI_2;
    traj[1] = get_time_var / M_PI_2 - 2;
  } else if (get_time_var >= M_PI && get_time_var < M_PI_2*3){
    traj[0] = get_time_var / M_PI_2 - 4;
    traj[1] = get_time_var / M_PI_2 - 2;
  } else {
    traj[0] = get_time_var / M_PI_2 - 4;
    traj[1] = - get_time_var / M_PI_2 + 4;
  }
